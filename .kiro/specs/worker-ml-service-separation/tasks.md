...
## Phase 1: ML Service Extraction (Weeks 1-2)

### 1. Set up ML Service project structure

- [ ] 1.1 Create ml-service directory with Python project layout
...
- [ ]* 1.2 Write unit tests for ML Service project structure
  - Test that FastAPI app initializes
  - Test that routers are registered
  - _Requirements: 3.1_

### 2. Implement model initialization and GPU detection

- [ ] 2.1 Create ModelManager for downloading and verifying models
...
- [ ] 2.2 Implement FastAPI lifespan with model initialization
...
- [ ]* 2.3 Write property test for model initialization
...
- [ ] 3.1 Implement object detection endpoint (/infer/objects)
...
- [ ] 3.2 Implement face detection endpoint (/infer/faces)
...
- [ ] 3.3 Implement transcription endpoint (/infer/transcribe)
...
- [ ] 3.6 Implement scene detection endpoint (/infer/scenes)
...
- [ ]* 4.2 Write unit test for health endpoint
  - Test healthy response when all models ready
  - Test degraded response when model fails
  - Test GPU info is included
  - _Requirements: 3.7_

### 5. Checkpoint - ML Service Complete

- [ ] 5.1 Ensure all ML Service tests pass
...
## Phase 2: Worker Service Integration (Weeks 2-3)

### 6. Add arq to backend and create job producer

- [ ] 6.1 Add arq and httpx dependencies to backend/pyproject.toml
  - Add arq ^0.26
  - Add httpx ^0.25
  - _Requirements: 4.1_

- [ ] 6.2 Create JobProducer with GPU/CPU routing
...
- [ ]* 6.3 Write unit test for JobProducer
  - Test GPU task routing to gpu_jobs
  - Test CPU task routing to cpu_jobs
  - Test job_id format "ml_{task_id}"
  - _Requirements: 4.1, 4.5_

### 7. Create ML Client for Worker Service

- [ ] 7.1 Create MLClient for HTTP communication with ML Service
...
- [ ]* 7.2 Write unit test for MLClient
  - Test HTTP requests to ML Service
  - Test response parsing
  - Test timeout handling
  - _Requirements: 18.1_

### 8. Create arq worker configuration

- [ ] 8.1 Create arq worker configuration with GPU/CPU mode
...
- [ ] 10.2 Add reconciler as periodic cron task in arq
  - Schedule reconciler to run every 5 minutes
  - _Requirements: 6.7_

- [ ]* 10.3 Write property test for reconciliation
  - **Property 7: Reconciliation Recovery**
  - **Validates: Requirements 6.1, 6.2, 6.3, 6.4, 6.5, 6.6**

### 11. Checkpoint - Worker Service Complete

- [ ] 11.1 Ensure all Worker Service tests pass
...
## Phase 3: API Service Updates (Week 3)

### 12. Update video discovery to auto-create tasks

- [ ] 12.1 Create VideoDiscoveryService
...
- [ ] 12.2 Integrate VideoDiscoveryService into video discovery workflow
...
- [ ]* 14.4 Write unit tests for task management endpoints
  - Test cancellation
  - Test retry
  - Test filtering and sorting
  - _Requirements: 10.1, 10.7, 10.8_

### 15. Add artifact schema validation

- [ ] 15.1 Create artifact payload schema models
...
- [ ] 15.2 Integrate schema validation into artifact persistence
...
- [ ]* 15.3 Write unit tests for artifact schema validation
  - Test valid payloads pass validation
  - Test invalid payloads are rejected
  - _Requirements: 23.3, 23.4_

### 16. Checkpoint - API Service Complete

- [ ] 16.1 Ensure all API Service tests pass
...
## Phase 4: Service Separation & Deployment (Week 4)

### 17. Create separate entry points

- [ ] 17.1 Create backend/src/main_api.py
...
- [ ] 17.2 Create backend/src/main_worker.py
...
- [ ]* 17.3 Write unit tests for entry points
  - Test API Service doesn't start arq consumer
  - Test Worker Service doesn't expose HTTP endpoints
  - _Requirements: 9.3, 9.4_

### 18. Create separate Dockerfiles

- [ ] 18.1 Create dev/Dockerfile.api
  - Lighter image without ML dependencies
  - Install only FastAPI, uvicorn, database drivers
  - Expose port 8000
  - _Requirements: 9.5_

- [ ] 18.2 Create dev/Dockerfile.worker
  - Include arq, httpx, database drivers
  - No ML model files
  - No HTTP server
  - _Requirements: 9.6_

- [ ] 18.3 Update dev/Dockerfile.ml (if needed)
  - Ensure all ML dependencies included
  - Ensure model cache directory configured
  - _Requirements: 9.7_

### 19. Update docker-compose.yml

- [ ] 19.1 Update docker-compose.yml with separate services
...
- [ ]* 19.2 Write integration test for docker-compose
  - Test all services start correctly
  - Test services can communicate
  - Test health endpoints respond
  - _Requirements: 9.1, 9.2_

### 20. Implement backward compatibility

- [ ] 20.1 Verify all existing REST endpoints still work
  - Test GET /videos
  - Test GET /tasks
  - Test GET /artifacts
  - Test GET /paths
...
- [ ]* 20.2 Write property test for backward compatibility
  - **Property 12: Backward Compatibility**
  - **Validates: Requirements 13.1, 13.2, 13.3, 13.4, 13.5, 13.6**

### 21. Implement observability

- [ ] 21.1 Add structured logging to all services
...
### 22. Checkpoint - Service Separation Complete

- [ ] 22.1 Ensure all integration tests pass
  - Run full test suite
...
### 23. Implement GPU semaphore concurrency control

- [ ] 23.1 Verify GPU semaphore limits concurrent requests
  - Test that max GPU_CONCURRENCY requests run simultaneously
  - Test that excess requests are queued
  - _Requirements: 7.1, 7.2, 7.3, 7.4, 7.5_

- [ ]* 23.2 Write property test for GPU semaphore
  - **Property 8: GPU Semaphore Concurrency Control**
  - **Validates: Requirements 7.1, 7.2, 7.3, 7.4, 7.5**

### 24. Implement horizontal scaling

- [ ] 24.1 Test multiple Worker Service instances
...
- [ ]* 26.2 Write property test for graceful shutdown
  - **Property 15: Graceful Shutdown**
  - **Validates: Requirements 20.1, 20.2, 20.3**

### 27. Implement job idempotency

- [ ] 27.1 Test job idempotency
  - Enqueue same job twice
  - Verify only one job is processed
  - Verify duplicate job is deduplicated
  - _Requirements: 19.1, 19.2, 19.3_

- [ ]* 27.2 Write property test for job idempotency
  - **Property 14: Job Idempotency**
  - **Validates: Requirements 19.2, 19.3**

### 28. Implement service isolation

- [ ] 28.1 Verify service isolation
...
- [ ]* 28.2 Write property test for service isolation
...
- [ ] 29.3 Documentation and cleanup
